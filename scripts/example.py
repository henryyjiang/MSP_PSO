...
predictor_batch = BasinHoppingBatch(forcefield, hops=10, steps=10, dr=0.6, optimizer='Adam', batch_size=1, perturbs=['pos'])
...
for i in range(0, max_iterations):
    ...
    #compositions = [[22, 22, 8, 8, 8, 8] for _ in range(8)]
    ...
    for i in range(len(total_list_batch)):
        for hop in total_list_batch[i]:
            hop_numbers.append(hop['hop'])
            unnormalized_losses.append(hop['unnormalized_loss'])

    plt.figure(figsize=(10, 6))
    plt.plot(hop_numbers, unnormalized_losses, marker='o', linestyle='-')
    plt.xlabel("Hop")
    plt.ylabel("Unnormalized Energy Loss")
    plt.grid(True)
    plt.show()
